...
class _EventServiceState extends State<EventService> {
  ...
  Future<void> addEvent() async {
    ...
    DocumentReference hotels = FirebaseFirestore.instance.collection('events').doc();
    await hotels.set({
      'name': name,
      'county': county,
      'city': city,
    ...
    try {
      ...
      await _showDialog('Success', 'Event added successfully.');

      // After the dialog is closed, navigate back
      Navigator.of(context).pop();
    } catch (e) {
      print('Error in addEvent: $e');
      await _showDialog('Error', 'Failed to add event.'); // Show error dialog
    }

  }

  // Helper methods for image upload
  Future<String?> uploadWebImage(Uint8List image, String eventId) async {
    String fileName = 'image_${DateTime.now().millisecondsSinceEpoch}.jpg';
    Reference storageRef = FirebaseStorage.instance.ref('gs://swipeaway-7195c.appspot.com/$eventId/$fileName');
    try {
  ...
  Future<String?> uploadMobileImage(File image, String eventId) async {
    String fileName = 'image_${DateTime.now().millisecondsSinceEpoch}.jpg';
    Reference storageRef = FirebaseStorage.instance.ref('gs://swipeaway-7195c.appspot.com/$eventId/$fileName');
    try {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text('Add Event', style: GoogleFonts.roboto()),
      ),
